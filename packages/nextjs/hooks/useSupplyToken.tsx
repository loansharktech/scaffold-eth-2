...
export function useSupplyToken(realm: Realm, market: Market) {
  ...
  const { status: approveTransStatus, error: approveTransError } = useWaitForTransaction({
    hash: tradeData.approveTx as any,
    chainId: parseInt(realm.contract.chainId),
  });

  console.log("approveTransError", approveTransError);

  const approveToken = useCallback(async () => {
    console.log("approveToken", _tokenApprove);
    if (!isLogin) {
      return login();
    }
    if (!_tokenApprove) {
  ...
  console.log("minteTransStatus", minteTransStatus);
