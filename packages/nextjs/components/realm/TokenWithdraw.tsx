...
const redeemmes = async (marketAddress: string, amount: number | undefined) => {
  ...
  const yourEthersContract = new EthersContract(
    ...
    if (amount) {
      ...
      const result = await wrappedContract.redeemUnderlying(amounts);
    }
}
      ...
      {typeof withdrawToken.amount === "undefined" ? (
        ...
        <Button
          ...
          onClick={() => {
            ...
            redeemmes(market.address, withdrawToken.amount)
          }}
        >
          Withdraw
        </Button>
